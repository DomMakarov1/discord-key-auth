...
-- UIScale for global UI sizing (persisted via settings)
local UIScale = Instance.new("UIScale")
UIScale.Name = "UIScale"
UIScale.Scale = (type(persistedConfig.uiScale) == "number")
    and math.clamp(persistedConfig.uiScale / 100, 0.5, 2)
    or 1
...
-- Discord invite button (left of center icon)
local function _uaGetDiscordInvite()
    return (getgenv and type(getgenv().UA_DiscordInvite) == "string" and getgenv().UA_DiscordInvite ~= "")
        and getgenv().UA_DiscordInvite
        or (UA_RUNTIME._discordInvite or CONFIG.DiscordInvite)
end
...
DiscordBtn.MouseEnter:Connect(function()
    tween(DiscordBtn, quickTween, { TextColor3 = Theme.AccentPrimary })
    local stroke = DiscordBtn:FindFirstChildOfClass("UIStroke")
    if stroke then tween(stroke, quickTween, { Color = Theme.AccentPrimary }) end
end)
DiscordBtn.MouseLeave:Connect(function()
    tween(DiscordBtn, quickTween, { TextColor3 = topBarIconWhite })
    local stroke = DiscordBtn:FindFirstChildOfClass("UIStroke")
    if stroke then tween(stroke, quickTween, { Color = Theme.Border }) end
end)

DiscordBtn.MouseButton1Click:Connect(function()
    playClickSound()
    local invite = _uaGetDiscordInvite()
    if invite and invite ~= "" then
        local clip = setclipboard or (syn and syn.write_clipboard) or toclipboard or writeclipboard
        if clip then
            local ok = pcall(clip, invite)
            if ok then
                notify("Discord invite copied to clipboard!", "success", 3)
            else
                notify("Discord: " .. invite, "info", 5)
            end
        else
            notify("Discord: " .. invite, "info", 5)
        end
    else
        notify("No Discord invite configured", "warning", 3)
    end
end)
...
local profileSection = createSection(leftCol, "PROFILE", UDim2.new(0, 0, 0, 0), UDim2.new(1, 0, 0, 185))
...
-- UI Scale stepper
createLabel("UI SCALE", profileSection, UDim2.new(0, 10, 0, 128))
...
local scaleMinus = create("TextButton", {
    ...
    create("UICorner", { CornerRadius = UDim.new(0, 6) }),
    create("UIStroke", { Color = Theme.Border, Thickness = 1, Transparency = 0.62 }),
})
...
local scaleBox = create("TextBox", {
    ...
    BackgroundColor3 = Theme.Surface,
    BorderSizePixel = 0,
    Text = string.format("%.2f", (persistedConfig.uiScale or 100) / 100),
    TextColor3 = Theme.Text,
    TextSize = 13,
    Font = Theme.FontMono,
    ClearTextOnFocus = false,
...
for _, btn in ipairs({scaleMinus, scalePlus}) do
    btn.MouseEnter:Connect(function()
        tween(btn, quickTween, { BackgroundColor3 = liftColor(Theme.Surface, 0.03) })
    end)
    btn.MouseLeave:Connect(function()
...
local function applyUiScale(percentValue)
    percentValue = math.clamp(math.floor(percentValue + 0.5), 50, 200)
    local scale = percentValue / 100
    local us = ScreenGui:FindFirstChild("UIScale")
    if us then
        us.Scale = scale
    end
    scaleBox.Text = string.format("%.2f", scale)
    persistedConfig.uiScale = percentValue
    savePersistedConfig()
end

scaleMinus.MouseButton1Click:Connect(function()
    playClickSound()
    applyUiScale((persistedConfig.uiScale or 100) - 5)
end)
...
scaleBox.FocusLost:Connect(function()
    ...
    if n then
        applyUiScale(math.floor(n * 100 + 0.5))
    else
        scaleBox.Text = string.format("%.2f", (persistedConfig.uiScale or 100) / 100)
    end
end)

refreshAccentUi, refreshCustomList = refreshAccentUi, refreshCustomList
;(function()
local accentSection = createSection(leftCol, "ACCENT PICKER", UDim2.new(0, 0, 0, 200), UDim2.new(1, 0, 0, 330))
...
local function revealMainUI(username)
    -- Sync settings from server (best-effort, runs in background)
    task.spawn(function()
        local ok, msg = syncSettingsFromServer()
        if ok and msg then
            if persistedConfig.nickname and persistedConfig.nickname ~= "" then
                PlayerNameLabel.Text = persistedConfig.nickname
            end
            if persistedConfig.accentPrimary and type(persistedConfig.accentPrimary) == "table" then
                local a = persistedConfig.accentPrimary
                if type(a.r) == "number" then
                    Theme.AccentPrimary = Color3.fromRGB(a.r, a.g, a.b)
                end
            end
            if persistedConfig.accentSecondary and type(persistedConfig.accentSecondary) == "table" then
            ...
            pcall(function()
                if refreshAccentUi then refreshAccentUi() end
