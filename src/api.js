...
export function createApi() {
  ...
  // Protected Admin.lua — requires valid JWT + HWID match (POST body: token, hwid).
  // HWID binding prevents a leaked token from being used on another device.
  app.post("/Admin.lua", async (req, res) => {
    try {
      const token = extractToken(req);
      if (!token) {
        return res.status(401).type("text/plain").send("-- Token required for Admin.lua. Run Loader.lua first.\n");
      }
      const hwid = (req.body && req.body.hwid) ? String(req.body.hwid).slice(0, 200) : null;

      const payload = await verifyScriptAccess(token, hwid);
      ...
      return res.status(401).type("text/plain").send("-- " + (err.message || "Access denied") + "\n");
    }
  });
